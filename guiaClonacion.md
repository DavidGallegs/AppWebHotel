...
4. Creamos la platilla .env: `cp backend/.env.example backend/.env`
5. Configuramos el archivo .env del backend con los datos de la base de datos:
...
6. Levantamos el docker compose: `docker compose -f docker-compose-dev.yml up -d --build`
7. Instalamos las dependencias:
    - `docker compose -f docker-compose-dev.yml exec frontend npm install`
    - `docker compose -f docker-compose-dev.yml exec backend composer install`
8. Para laravel generar llave de seguridad y migrar tablas:
    - `docker compose -f docker-compose-dev.yml exec backend php artisan key:generate`
    - `docker compose -f docker-compose-dev.yml exec backend php artisan migrate`
9. Al estar en WSL:Ubuntu otorgamos permisos de edición: `sudo chown -R $USER:$USER .`
