...
const AdminContent = () => {
  ...
  mutationFn: async ({ id, accion, tipo }: {
    id: string | number,
...
const aprobarReserva = useMutation({
  mutationFn: async (id: string | number) => {
    return await api.patch(`/admin/reservations/${id}/approve`);
  },
  onSuccess: () => {
                      ...
                      <tr key={res.id}>
                        ...
                        <td>
                          <div className="action-buttons">
                            {res.status === 'pending' ? (
                              <>
                                <button type="button" className="btn-outline btn-blue" onClick={() => aprobarReserva.mutate(res.id)}>Aprobar</button>
                                <button type="button" className="btn-outline btn-red" onClick={() => rechazarReserva.mutate(res.id)}>Rechazar</button>
                              </>
                            ) : res.status === 'approved' ? (
                              <>
