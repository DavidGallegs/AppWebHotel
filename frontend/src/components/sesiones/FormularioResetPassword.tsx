...
export function FormularioResetFinal({ token, email }: { token: string, email: string }) {
    ...
    return (
        <form onSubmit={manejarReset} className="form-reset">
            <div className="form-group">
                ...
                <input id="nuevo-password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mínimo 8 caracteres" />
            </div>
            <div className="form-group">
                <label htmlFor="confirmar-password">Confirmar Contraseña</label>
                <input id="confirmar-password" type="password" required value={confirmar} onChange={(e) => setConfirmar(e.target.value)} placeholder="Repite la contraseña" />
            </div>
            <button type="submit" disabled={cargando} className="btn-submit" aria-busy={cargando}>
